...
// Migrado para usar o hook useQuestions (centralização de operações de questões).
...
function QuestionPreviewHeader() {
        return (
                <div className="flex flex-col justify-center text-left w-full sm:max-w-[700px] min-h-[220px] p-5 self-center">
                        ...
                        <div className='text-center text-sm text-stone-500 mt-2'>Exemplo da listagem oficial de questões do backend, via useQuestions</div>
                </div>
        )
}

const LIMIT: QuestionFilters['limit'] = '5'
const PREVIEW_LEVEL: QuestionFilters['level'] = 'n5'
const PREVIEW_TOPIC: QuestionFilters['topic'] = 'listening'
const PREVIEW_ANSWER_STATUS: QuestionFilters['answer_status'] = 'all'

export default function QuestionPreview() {
        const { authStatus } = useRequireAuth()
        const { questions, total, isLoading, error, getQuestionList } = useQuestions()
        ...
        useEffect(() => {
                void getQuestionList({
                        level: PREVIEW_LEVEL,
                        topic: PREVIEW_TOPIC,
                        answer_status: PREVIEW_ANSWER_STATUS,
                        limit: LIMIT,
                        page,
                })
                                ...
                                {isLoading ? (
                                        ...
                                        <div className='space-y-5'>
                                                <div className='border-2 border-stone-200 rounded-md p-5 my-3'>
                                                        <div className='font-bold text-2xl p-2 self-center'>Rota /levels/{PREVIEW_LEVEL}/topics/{PREVIEW_TOPIC}/questions</div>
                                                        <div className='text-sm text-stone-500 px-2 pb-2 flex items-center gap-4'>
                                                                <div>Answer status: {PREVIEW_ANSWER_STATUS}</div>
                                                ...
                                                <div className='border-2 border-stone-200 rounded-md p-5 bg-white shadow-sm'>
                                                        ...
                                                        <pre className='whitespace-pre-wrap text-sm overflow-auto'>{JSON.stringify(questions, null, 2)}</pre>
                                                </div>
                                        </div>
                                ) : null}
                        </div>
